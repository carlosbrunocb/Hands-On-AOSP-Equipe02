...
/**
 * List of screen lock type options that are available in ChooseLockGeneric.
 * Provides the key and
 * the associated quality, and also some helper functions to translate between
 * them.
 */
public enum ScreenLockType {
  ...
  /**
   * The highest quality for the given type of lock. For example, in the case of
   * password, the
   * default quality is PASSWORD_QUALITY_ALPHABETIC, but the highest possible
   * quality is
  ...
  /**
   * Gets the screen lock type for the given quality. Note that this method
   * assumes that a screen
   * lock is enabled, which means if the quality is
   * {@link DevicePolicyManager#PASSWORD_QUALITY_UNSPECIFIED}, the returned type
   * will be
